...
class Vincy(commands.Cog):
    ...
    async def cog_check(self, ctx):
        if ctx.guild.id == self.guild["id"]:
            ...
            await ctx.send(
                ...
                if self.bot.lang.get_user(ctx.author.id) != "it"
    ...
    async def vincyrules(self, ctx):
        embed = (
            ...
            .add_field(
                ...
                value="Lo spam in questo server è proibito.\n"
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if int(payload.message_id) == int(self.agree["message_id"]):
            if str(payload.emoji) == str(self.agree["emoji"]):
                ...
                await self.bot.get_channel(595327311012823045).send(
                    f"**{payload.member}** ha accettato le regole."
                )
            else:
                await self.bot.http.remove_reaction(
                    int(self.agree["channel_id"]),
                    int(self.agree["message_id"]),
                    payload.emoji,
                    payload.user_id,
                )
    ...
    async def on_message(self, message):
        if message.channel.id == int(self.agree["channel_id"]):
            if (
                message.content == "accetto"
                ...
                await self.bot.get_channel(595327311012823045).send(
                    f"**{message.author}** ha accettato le regole."
                )
            else:
                try:
                ...
                if kicks < 5:
                    ...
                    await message.author.kick(
                        reason=f"Invece di accettare le regole ha scritto questo:\n{message.content}"
                    )
                    await self.bot.get_channel(595327311012823045).send(
                        f"**{message.author}** non ha accettato le regole, il kick era meritato.\nAveva scritto questo:\n{message.content}"
                    )
                else:
                    await message.delete()
                    ...
                    await self.bot.get_channel(595327311012823045).send(
                        f"**{message.author}** non ha accettato le regole, stavolta si è meritato il ban!\nAveva scritto questo:\n{message.content}\n\nhttps://imgur.com/V4TVpbC"
                    )
