...
class MongoDBClient(ApiClient):
    def __init__(self, bot):
        mongo_uri = bot.config["mongodb"]
        if mongo_uri is None:
            logger.critical("Un MongoDB è necessario per il corretto funzionamento del bot.")
            raise RuntimeError
        ...
        try:
            ...
            logger.critical(
                "La tua stringa di connessione potrebbe essere copiata male, prova a ricopiarlo. "
                "Altrimenti controlla il messaggio seguente:"
            )
            logger.critical(e)
            sys.exit(0)
